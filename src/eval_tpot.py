...
def eval_tpot(openml_id, exam_id, time_budget, n_jobs=8):
    ...
    #pipeline building
    exe_code = reg.export()
    pipeline_part = exe_code[exe_code.find("exported_pipeline"):]
    begin_pos = pipeline_part.find("make_pipeline")
    end_pos = pipeline_part.find('#')
    pipeline_building = pipeline_part[begin_pos:end_pos].replace('\n', '').replace(' ', '')
